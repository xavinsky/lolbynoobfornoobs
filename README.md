# LoL by Noob for Noobs

Un guide de League of Legends fait par un noob, pour les noobs.

> ⚠️ Fait par un débutant : le site peut contenir des erreurs. Chaque donnée cite sa source pour pouvoir la vérifier.

Quand on débute, on trouve partout des chiffres, des tier lists et des builds, mais presque rien qui explique *pourquoi* : pourquoi ce champion gagne contre celui-là, ce que fait vraiment un sort, à quoi sert cet objet, ce que veut dire un pourcentage de victoire. Les guides s'adressent à des joueurs qui savent déjà. Faute de trouver cette documentation, je la fais moi-même, comme je peux, au fil de ce que j'apprends, et je la partage : si elle m'aide, elle peut aider d'autres débutants.

Le site est publié sur GitHub Pages : **https://xavinsky.github.io/lolbynoobfornoobs/**

## Ce qu'on y trouve

- **Champions** : la liste avec ce qui compte pour choisir (type de dégâts, phase de force, rôles, popularité), et pour chacun sa fiche : points forts et contre, compétences avec leur animation et leurs mécaniques expliquées, vidéos de présentation, mises à jour du personnage, courbe de puissance selon la durée de partie.
- **Counters** : pour un champion, les duels nettement gagnés ou perdus chez les meilleurs joueurs (Master et au-dessus), avec la mesure, sa netteté, et les explications des joueurs coréens de Namu Wiki traduites automatiquement, confrontées à la mesure.
- **Bases** : les compétences classées par ce qu'elles font (comment on vise, qui est touché, quel type de dégâts, quelle portée, quel effet), les objets, leurs effets et leurs statistiques expliqués un par un.
- **Sources** : d'où vient chaque information, à quelle date, avec quelle règle de calcul, et ce que veut dire chaque valeur affichée. Rien n'est affiché sans sa source.

## Comment c'est fait

Des pages statiques, sans serveur : le site s'ouvre aussi bien en local qu'en ligne. Ce dépôt est généré depuis un brouillon de travail privé, où j'avance sur d'autres sujets (statistiques par palier, matchmaking) avant de les publier ; il n'est pas édité à la main.

Les données viennent du wiki League of Legends, de lolalytics, de Data Dragon (Riot Games) et de Namu Wiki ; le détail et les dates de récupération sont sur la page Sources.

## Mettre à jour les statistiques

Les taux de victoire, de pick et de ban se périment à chaque patch : ils sont relevés depuis le brouillon privé et
republiés ici avec leur date et leur patch, visibles sur la page Sources.

## Contribuer

Toute contribution est la bienvenue, même un détail : voir [CONTRIBUTING.md](CONTRIBUTING.md), les issues du dépôt, ou le formulaire sans compte de la section « Contribuer » du site.

## Licences

Code et textes du site : [licence MIT](LICENSE). Le fichier `LICENSE` détaille aussi les contenus tiers et leurs conditions : textes traduits ou résumés du wiki League of Legends sous [CC BY-SA 3.0](https://creativecommons.org/licenses/by-sa/3.0/) (page d'origine citée), explications de matchups traduites de Namu Wiki sous [CC BY-NC-SA 2.0 KR](https://creativecommons.org/licenses/by-nc-sa/2.0/kr/), données de jeu, icônes et vidéos de Riot Games, chiffres de lolalytics, conseils extraits de vidéos YouTube citées.

LoL by Noob for Noobs was created under Riot Games' "Legal Jibber Jabber" policy using assets owned by Riot Games. Riot Games does not endorse or sponsor this project.
