...
const GLOBAL_SOURCES = [
  ...
  },
  {
    ...
    method: "Généré par script/update_tiers.py (livré avec le site, Python seul) à partir des pages « tier list » de lolalytics, une page par palier et par rôle (Ranked Solo/Duo, toutes régions, patch courant), lues en une requête HTTP chacune et décodées sans exécuter de JavaScript : par champion et rôle, taux de victoire, de pick et de ban, nombre de parties ; par palier et rôle, nombre de parties analysées. Sur le site, le sélecteur de paliers est dans l'en-tête (réglage de tout le site, mémorisé, transmis de page en page) ; les paliers cochés (Fer, Bronze, Argent, Or) sont combinés : WR = moyenne des WR pondérée par les parties du champion à chaque palier ; pick et ban = moyennes pondérées par les parties analysées de chaque palier. Tout décocher revient à cocher les quatre : le site s'adresse aux joueurs de ces paliers, la référence Émeraude+ (mécanisme « roles ») ne sert qu'en l'absence du fichier de paliers. La date et le patch sont ceux inscrits dans le fichier généré.",
