// Mécanismes de sourcing des données affichées : source, ce qui en est repris, règle de transformation, date.
// Généré depuis le brouillon privé — ne pas éditer.
const GLOBAL_SOURCES = [
  {
    ...
    "method": "Palier Émeraude+ des tier lists de lolalytics (Ranked Solo/Duo, toutes régions), relevé avec les autres paliers (mécanisme « rolesTiers ») : pour chaque rôle listé du champion, taux de victoire, de pick et de ban tels quels, et la note de tier de lolalytics, écrite avec ses quinze paliers, de S+ à D−.",
    "url": "https://lolalytics.com/lol/tierlist/?lane=top&tier=emerald_plus",
    "retrieved": "2026-09-14",
    "patch": "16.18 / 26.18",
    "caveat": "Instantané figé à cette date — se périme au rythme des patchs (~2 semaines). Le taux de victoire de lolalytics compte le palier de chaque joueur : la moyenne des champions en Émeraude+ est d'environ 51,7 %, pas 50 %. La liste des rôles d'un champion (258 rôles pour 173 champions) n'est pas recalculée à chaque relevé : lolalytics classe aussi des rôles très marginaux (425 rôles en Émeraude+), que le site ne liste pas."
  },
  {
    "id": "rolesCombined",
    "label": "WR / PR / Ban affichés hors des pastilles de rôle (vue Liste, tous filtres)",
    "fields": [],
    "method": "Valeur calculée à l'affichage, pas stockée — dérivée du mécanisme \"roles\" ci-dessus, et affichée quel que soit le filtre de rôle actif (pas seulement \"Tous\"). PR = somme des PR de chaque rôle joué par le champion (même base de parties par bracket, donc la somme donne le pick rate toutes lignes confondues). WR = moyenne des rôles pondérée par leur PR respectif (poids = volume de parties de ce rôle). Ban = UNE seule valeur par champion (on bannit un champion, pas un rôle) : celle du rôle où le champion a le plus haut PR — la source donne la même valeur sur chaque rôle (écart constaté entre rôles : nul, sur les 68 champions joués à plusieurs rôles).",
    "url": null,
    "retrieved": null,
  ...
  },
  {
    ...
    "fields": [
      "data/roles_tiers.js : ROLES_TIERS.champions[clé][rôle][palier] (wr, pr, br, games, grade), ROLES_TIERS.tiers[palier].lanes[rôle].analysed"
    ],
    "method": "Tier lists de lolalytics, une par palier et par rôle (Ranked Solo/Duo, toutes régions, patch courant) : par champion et rôle, taux de victoire, de pick et de ban, nombre de parties ; par palier et rôle, nombre de parties analysées. Sur le site, le sélecteur de paliers est dans l'en-tête (réglage de tout le site, mémorisé, transmis de page en page) ; les paliers cochés (Fer, Bronze, Argent, Or) sont combinés : WR = moyenne des WR pondérée par les parties du champion à chaque palier ; pick et ban = moyennes pondérées par les parties analysées de chaque palier. Tout décocher revient à cocher les quatre : le site s'adresse aux joueurs de ces paliers, la référence Émeraude+ (mécanisme « roles ») ne sert qu'en l'absence du fichier de paliers. La date et le patch sont ceux du relevé.",
  ...
  },
  {
    ...
    "method": "Vidéos de présentation cherchées sur YouTube pour chaque champion, sur trois chaînes précises reconnues par leur identifiant de chaîne (jamais par leur nom affiché, qui peut être imité) : « Focus sur {nom} » de League of Legends - France (officielle), « {nom} Champion Spotlight » de League of Legends (chaîne globale officielle), « 3 Minute {nom} Guide » de 3 Minute League of Legends (chaîne communautaire, pas Riot, étiquetée comme telle sur la fiche). Le premier résultat pertinent de chaque recherche est retenu, avec sa date de publication et sa durée.",
    "url": null,
    "retrieved": "2026-09-08",
  ...
  },
  {
    ...
    "fields": [
      "data/mechanics.js : ABILITY_MECHANICS[id][passive|q|w|e|r]"
    ],
    "method": "Gabarits de données du wiki League of Legends, un par sort : Template:Data {Nom anglais}/{I|Q|W|E|R} (I = passif). Champs repris : targeting, affects, damagetype, spellshield, spelleffects, cast time, cooldown, range (valeurs traduites par une table fixe à l'affichage : Direction, Unit→Unité ciblée, Location→Zone au sol, Physical→Physiques, True→Bloqué pour le bouclier de sort…), ce qui interrompt l'incantation ou la canalisation, et la liste « notes » (puces), débarrassée du balisage wiki mais laissée en anglais, sans reformulation.",
  ...
  },
  {
    ...
    "fields": [
      "data/mechanics_fr.js : ABILITY_MECHANICS_FR[id][*].notes / .interrupts"
    ],
    "method": "Traduction automatique des notes anglaises du mécanisme « mechanics » par un modèle de traduction exécuté localement (translategemma:12b), texte par texte, avec : 1) les noms anglais des sorts remplacés au préalable par leurs noms officiels français (Data Dragon) ; 2) un glossaire imposé (bouclier de sort, délai de récupération, canalisation, temps d'incantation, attaque de base, à l'impact, contrôle de foule, ténacité, dégâts bruts/magiques/physiques, sbire, tourelle, balise, intouchable, étourdissement, immobilisation, ralentissement, projeter en l'air, repousser, Châtiment, Faille de l'invocateur…). Une note dont l'anglais change est retraduite, les autres conservées. Contrôle automatique : une traduction qui ressemble à une dérive du modèle (préambule « Voici… », texte bien plus long que l'anglais, caractères étrangers) est retraduite avec une consigne plus stricte ; si elle reste suspecte, la note repasse en anglais. L'anglais d'origine reste affiché sous la traduction (repli « Texte d'origine »).",
  ...
  },
  {
    ...
    "fields": [
      "data/item_effects.js : ITEM_EFFECTS[id].keys / .effects / .sr"
    ],
    "method": "Module de données des objets du wiki League of Legends (Module:ItemData/data), rapproché de Data Dragon par identifiant : pour chaque objet de la Faille, ses effets nommés (passifs pass/pass2/pass3, actif act, consommable consume) avec leur texte anglais, leurs mots-clés liés {{tip|…}} et leur nom français (balises <passive>/<active> de la description Data Dragon, dans l'ordre). Classement en clés (vocabulaire fixe du site) par des règles fixes : un mot-clé lié du wiki (ex. grievous wounds, on-hit, slow, shield, heal, stasis, execute, energized), un motif sur le texte anglais de l'effet (ex. « armor reduction », « burn », « movement speed », « stacks … permanent »), ou un motif sur la description française hors bloc de statistiques (ex. « Lame enchantée », « Fendoir », « Hémorragie », « à l'impact », « ralenti », « alliés … soigne »). Une purge qui nomme les contrôles qu'elle retire ne reçoit pas ces contrôles. Cumuls : « stacking » (croissance permanente : permanent, infini, définitivement, ou charges gagnées sur les éliminations et sbires sans expiration), sinon « stacks » (charges temporaires) ; un objet à croissance permanente ne porte pas les deux. Disponibilité sur la Faille : modes du wiki (« classic sr 5v5 »), l'entrée wiki étant retrouvée par identifiant, sinon par nom anglais (Data Dragon en_US) — les objets d'Arena et de la partie rapide (objets Gardien, Jus chapi-chapo) que Data Dragon marque maps[11] sont écartés (17 objets). Avant les règles, le texte est nettoyé des passages qui nomment un effet sans que l'objet l'applique (condition de déclenchement « ralentir ou immobiliser un champion vous octroie… », résistance aux ralentissements, balise camouflée, exclusions entre parenthèses, exécution de sbires) ; une courte table de corrections à la main (vérifiées sur la description en jeu) ajoute ou retire des clés là où le texte trompe encore la règle (objets déclenchés par un soin ou un bouclier, amplifications, charges permanentes ou temporaires).",
  ...
  },
  {
    ...
    "fields": [
      "data/mechanics.js : ABILITY_MECHANICS[id][*].tax"
    ],
    "method": "Dérivé des gabarits wiki du mécanisme « mechanics » (mêmes 865 gabarits), dans un vocabulaire fixe du site. Ciblage : champ `targeting` découpé sur « / », « , », « | », gardé s'il vaut auto, direction, passive, location, unit, proximity, vector ou varied. Cibles : champ `affects`, ramené à enemies / self / allies / structures (turrets, turret ruins, allied turrets compris) / terrain / wards / monsters / none. Type de dégâts : champ `damagetype`, jetons magic (magical compris), physical, true. Bouclier de sort, projectile, ancrage (grounded), knockdown, parade (parry), silence, appel à l'aide (callforhelp) : valeur true / false / special / unknown du champ correspondant, telle quelle. Portée : première valeur numérique de `target range` (sinon `range`), « Global » = globale, puis tranche : ≤ 150 sur soi, 151-450 mêlée, 451-700 courte, 701-1000 moyenne, 1001-1500 longue, 1501-5000 très longue, au-delà globale (seuils choisis d'après la distribution des 692 portées officielles Data Dragon). Effets : mots-clés liés {{tip|…}} présents dans les champs description*, leveling* et blurb* du gabarit — jamais dans les notes, où un mot-clé signale souvent une interaction ou une immunité, pas un effet du sort — ramenés à une clé par une table de synonymes (ex. stun/stuns → stun ; knock up, knock back, knock aside, pull → airborne ; heal/heals/healing → heal ; invisibility, camouflage, vanish → stealth ; dispel → cleanse ; lunge → dash) ; tout mot-clé hors table est ignoré. Cumuls : sur le même texte, « stacking » (croissance permanente) si le sort accumule des charges et que le texte dit permanent / infini / sans limite / pour le reste de la partie, ou si les charges viennent des éliminations, sbires ou monstres sans durée d'expiration ; sinon « stacks » (charges temporaires) dès qu'une accumulation est mentionnée ; deux listes tenues à la main corrigent la règle là où le texte trompe (croissance permanente sans le mot « stack » : Sion W, Thresh, Senna, Bard, Shyvana, Rengar, Garen W, Swain, Sona, Draven, Kayle, Kai'Sa, Viktor, Kha'Zix R ; faux positifs retirés : Ahri, Alistar, Darius R, Irelia, Mordekaiser, Sejuani E, Talon, Tristana W, Yuumi, Zeri R, Bel'Veth), chaque entrée vérifiée sur la fiche wiki du sort. Définitions des contrôles de foule : page wiki « Crowd control », résumé traduit. Une table de corrections à la main (vérifiées sur la fiche wiki) retire les effets que le texte nomme sans que le sort les applique : ce qui interrompt un dash (immobilisation, ancrage, polymorphie, knockdown), « la cible n'a pas besoin d'être visible », purges de ralentissements, attaques qui ne peuvent pas faire de coup critique, « vanish » (intouchable, pas furtif), exécutions de sbires.",
    "url": "https://wiki.leagueoflegends.com/en-us/Template:Ability_data",
    "retrieved": "2026-09-12",
  ...
  },
  {
    ...
    "fields": [],
    "method": "Page officielle https://www.leagueoflegends.com/fr-fr/how-to-play/ : ses sections (« Détruire la base », « Se frayer un chemin », « À l'assaut de la jungle », « Choisissez votre voie », « Renforcez votre champion », « Déverrouillez vos compétences ») : titres, textes et vignettes repris tels quels. Carte : minimap officielle Data Dragon (img/map/map11.png). Icônes des structures, sbires et monstres : fichiers image du wiki (noms vérifiés un par un), chaque icône renvoyant à la page wiki (en) de l'élément.",
  ...
  },
  {
    ...
    "fields": [],
    "method": "Notes de patch officielles de Riot, en français, archivées telles quelles (titre, date de publication, texte) à partir de leur page d'index. Matière brute pour l'investigation, non chargée par les pages de l'outil.",
  ...
  },
  {
    ...
    "method": "Table « List of champions » du wiki League of Legends : date de sortie de chaque champion, lue ligne par ligne.",
